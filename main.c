...
int main() {
  ...
  Frame frame = {.frame_delay = 3, .frame_index = 0, .frame_delay_counter = 0};
  Sprite current_sprite = set_current_sprite(&sprites, 0);
  // Load the textures
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsKeyDown(KEY_RIGHT)) {
      ...
      current_sprite = select_texture_sprite_by_condition(&sprites, MOVE_RIGHT);
      ...
      current_sprite = select_texture_sprite_by_condition(&sprites, IDLE);
    ...
    //                (Rectangle){sky_pos.x, sky_pos.y, w, h}, (Vector2){0, 0},
    //                0, WHITE);
    // DrawTexturePro(
    //     t_jungle_bg,
    ...
    //                (Rectangle){tree_and_bushes_pos.x, tree_and_bushes_pos.y,
    //                w,
    ...
    // } else {
    //   DrawTextureRec(t_samurai_idle, samurai_rec_idle, samurai_idle_pos,
    //   WHITE);
    // }
    EndDrawing();
  }
